...
topics = [
    # Batch 1 - AI Models & Coding Tools
    ...
    # Batch 3 - Data, Security, Business
...
existing = set(f.replace('.md', '') for f in os.listdir(blog_dir) if f.endswith('.md'))
...
print(f"\nTotal requested: {len(topics)}")
print(f"Already exist: {len(topics) - len(available)}")
print(f"Available: {len(available)}")

# Write available topics to file
with open('/data/astro-blog/scripts/batch_topics.txt', 'w') as f:
    for t in available:
        f.write(t + '\n')
