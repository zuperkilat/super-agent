---
title: 'Idempotency dalam Agent Actions'
description: 'Membangun agent yang aman untuk retry tanpa duplicate side effects.'
pubDate: '2026-07-27'
...
Setiap action dapatkan unique idempotency key dari user atau system. Service deduplicate by key.

## Recovery Actions

Jika action succeed tapi response lost, retry dengan same key. Service detect duplicate dan return cached result.

## Implementation

Database unique constraint on (idempotency_key, action_type). Check before execute.
## FAQ
...
- [Idempotency Patterns](https://aws.amazon.com/builders-library/)
---
